...
function TabViewer(props: ITabViewerProps) {
    ...
    let tabPanes = props.tabs.map((tab: ITabPaneProps, pIndex) => {
      paginationItems.push(
        <PageItem key={"tab" + String(pIndex)} onClick={() => setKey(props.tvId + "event-key" + String(pIndex))} active={pIndex === Number(key)}>
          {tab.title}
        </PageItem>,
      );

      return (
        <TabPane bsPrefix='pane' key={"pane" + String(pIndex)} eventKey={props.tvId + "event-key" + String(pIndex)} unmountOnExit={true} transition={false}>
            {tab.children.render()}
        </TabPane>
      );
    });
